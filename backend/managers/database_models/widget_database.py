...
if __name__ == "__main__":
  ...
  try:
    ...
    pass
  engine = create_engine(f"sqlite:///{path}") #should create a .db file next to this one
  db = WidgetDb
  db.open(engine) #creates all the tables in widgets
  Session = sessionmaker(bind=engine)
  session = Session()
  row = db.models.get("widget-params-base")(widget_class='display', description="Top Level Display")
  session.add(row)
  session.commit() # need to commit to get widget's id
  display_id = row.id
  for table in db.models:
    if table.startswith('widget-params'):
      row = db.models.get("widget-params-base")(parent_id=display_id, widget_class=table, x=10, y=10, width=10, height=10, description="Some description")
      session.add(row)
      session.commit() # need to commit to get widget's id
    if table =='widget-params-alarm-config': 
      idx = row.id
      row = db.models.get(table)(id=idx, value_tag="testTag", enable_expression="Test expression", ctrl_btn_style = "TestStyle")
      session.add(row)
      session.commit()
    if table =='widget-params-check-box': 
      idx = row.id
      row = db.models.get(table)(id=idx, value_tag="TestTag",  on_change="Test OnChange",  initial_tag="TestInitialTag")
      session.add(row)
      session.commit()
    if table =='widget-params-ctrl-button': 
      idx = row.id
      row = db.models.get(table)(id=idx,
                                            label ="Hello",
                                            image = "testImage",
